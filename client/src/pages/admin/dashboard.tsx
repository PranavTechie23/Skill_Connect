...
const AdminDashboard: React.FC = () => {
  ...
  const loadUsers = async () => {
    try {
      ...
      setRecentUsers((usersData || []).slice(0, 5).map(user => ({
        ...user,
  ...
  return (
    <ErrorBoundary>
      <div className="relative">
        ...
        <div className={`min-h-screen transition-colors duration-300 flex flex-col ${darkMode ? 'bg-gray-900' : 'bg-gray-50'}`}>
          {loading && (
          ...
          {/* Top Bar */}
          <div className={`fixed top-0 left-0 right-0 z-50 ${darkMode ? 'bg-gray-800/90' : 'bg-white/90'} border-b ${darkMode ? 'border-gray-700' : 'border-gray-200'} backdrop-blur-xl`}>
